...
export interface DataseaRenderFrame {
  time: number;
  phaseTime?: number;
  phase: string | null;
...
export interface DataseaRenderer {
  readonly ready: Promise<void>;
  render(frame: DataseaRenderFrame): void;
  resize(): void;
  dispose(): void;
}
...
function disposeObject(root: THREE.Object3D, keepMaterial?: THREE.Material) {
  root.traverse((object) => {
    ...
    materials.forEach((entry) => {
      if (entry !== keepMaterial) entry.dispose();
    });
  });
}

export function createDataseaRenderer(
  ...
  const scene = new THREE.Scene(),
    fog = new THREE.Fog(0x01050b, 10, 240);
  ...
  scene.add(root);
  const environmentSH = [
    0.102, 0.074, 0.083, -0.016, -0.013, -0.02, 0.005, 0.005, 0.011, 0.02,
    0.021, 0.036, -0.04, -0.031, -0.041, -0.061, -0.044, -0.051, -0.014, -0.011,
    -0.013, 0.059, 0.043, 0.051, 0.029, 0.022, 0.025,
  ].reduce<THREE.Vector3[]>((values, value, index) => {
    if (index % 3 === 0) values.push(new THREE.Vector3());
    values.at(-1)!.setComponent(index % 3, value);
    return values;
  }, []);
  const uniforms = {
    uTime: { value: 0 },
    ...
    uSH: { value: environmentSH },
    uEnvT: { value: new THREE.Matrix3() },
  };
  ...
  const moteGeometry = new THREE.BufferGeometry(),
    moteCount = 280,
    positions = new Float32Array(moteCount * 3),
    velocities = new Float32Array(moteCount * 3),
    tints = new Float32Array(moteCount * 3),
    sizes = new Float32Array(moteCount);
  let randomState = 0x4e4f5249;
  const random = () => {
  ...
  for (let index = 0; index < moteCount; index++) {
    ...
    positions[offset] = -28 + random() * 56;
    positions[offset + 1] = -20 + random() * 40;
    positions[offset + 2] = -70 + random() * 66;
    velocities[offset] = (random() - 0.5) * 0.28;
    velocities[offset + 1] = 0.08 + random() * 0.24;
    velocities[offset + 2] = (random() - 0.5) * 0.18;
    const star = index % 7 === 0;
    tints[offset] = star ? 0.78 : 0.22;
    tints[offset + 1] = star ? 0.92 : 0.7;
    tints[offset + 2] = 1;
    sizes[index] = star ? 3.4 : 1.4 + random() * 1.8;
  }
  ...
  moteGeometry.setAttribute("aTint", new THREE.BufferAttribute(tints, 3));
  moteGeometry.setAttribute("aSize", new THREE.BufferAttribute(sizes, 1));
  const moteUniforms = { uTime: { value: 0 }, uFade: { value: 0 } },
  ...
  const bloom = new UnrealBloomPass(new THREE.Vector2(1, 1), 0, 0.4, 0.55);
  composer.addPass(bloom);
  const temporal = new DataseaTemporalPass(camera);
  composer.addPass(temporal);
  const grade = new ShaderPass({
    uniforms: { tDiffuse: { value: null }, uGrade: { value: 0 } },
    vertexShader:
      "varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}",
    fragmentShader:
      "uniform sampler2D tDiffuse;uniform float uGrade;varying vec2 vUv;vec3 aces(vec3 x){return clamp((x*(2.51*x+.03))/(x*(2.43*x+.59)+.14),0.,1.);}void main(){vec3 c=texture2D(tDiffuse,vUv).rgb;gl_FragColor=vec4(mix(c,aces(c),uGrade),1.);}",
  });
  composer.addPass(grade);
  const fxaa = new ShaderPass(FXAAShader);
  composer.addPass(fxaa);
  let disposed = false,
    loadedRoot: THREE.Object3D | null = null,
    width = 0,
    height = 0,
    pixelRatio = 0;
  let previousFrameKey: string | null = null,
    settledWaveKey: string | null = null;
  const manager = new THREE.LoadingManager();
  const gltfLoader = new GLTFLoader(manager).setMeshoptDecoder(MeshoptDecoder);
  const ready = Promise.allSettled([
    gltfLoader.loadAsync(GLB),
    loadTexture(new THREE.TextureLoader(manager), NEBULA),
    loadTexture(new THREE.TextureLoader(manager), HEIGHT),
  ]).then((results) => {
    const [gltfResult, nebulaResult, heightResult] = results;
    if (results.some((result) => result.status === "rejected")) {
      if (gltfResult.status === "fulfilled")
        disposeObject(gltfResult.value.scene);
    ...
    if (
      gltfResult.status !== "fulfilled" ||
      nebulaResult.status !== "fulfilled" ||
      heightResult.status !== "fulfilled"
    )
      throw new Error("Datasea assets failed to load");
    const gltf = gltfResult.value,
      nebula = nebulaResult.value,
      heightMap = heightResult.value;
    if (disposed) {
    ...
    nebula.wrapS = nebula.wrapT = THREE.MirroredRepeatWrapping;
    heightMap.wrapS = heightMap.wrapT = THREE.MirroredRepeatWrapping;
    uniforms.uNebula.value = nebula;
    ...
    modelRoot.traverse((object) => {
      if (object instanceof THREE.Mesh) {
        ...
        object.material = object.geometry.getAttribute("uv")
          ? gasMaterial
          : shellMaterial;
    ...
    modelRoot.scale.setScalar(70 / Math.max(size.x, size.y, size.z, 1));
    root.add(modelRoot);
  });
  const resize = () => {
    const nextWidth = Math.max(1, canvas.clientWidth),
      nextHeight = Math.max(1, canvas.clientHeight),
      ratio = Math.min(devicePixelRatio || 1, 2);
    if (nextWidth === width && nextHeight === height && pixelRatio === ratio)
      return;
    width = nextWidth;
    height = nextHeight;
    pixelRatio = ratio;
    previousFrameKey = settledWaveKey = null;
  ...
  let previousTime = Number.NaN,
    previousPosition = new THREE.Vector3(Number.POSITIVE_INFINITY, 0, 0),
    previousQuaternion = new THREE.Quaternion();
  const render = (frame: DataseaRenderFrame) => {
    if (disposed) return;
    resize();
    // A parked wave has fixed scene time. Reuse its fully converged framebuffer,
    // while the independent game canvases continue receiving every input/frame.
    // Any camera, phase, tune value, time, or viewport change invalidates the key.
    const frameKey = JSON.stringify(frame);
    if (frame.phase === "waves" && settledWaveKey === frameKey) return;
    ...
    uniforms.uEnvT.value.getNormalMatrix(camera.matrixWorldInverse);
    const cosmic = frame.phase === "cosmic",
      convergence = frame.phase === "converge",
      local = frame.phaseTime ?? frame.time;
    const reveal =
        frame.reveal ??
        (loadedRoot ? Math.min(1, uniforms.uReveal.value + 0.012) : 0),
      dissolve =
        frame.dissolve ??
        (cosmic ? Math.max(0, Math.min(0.92, local / 46)) : 0),
      gradeValue = frame.grade ?? (cosmic ? 1 : 0.35),
      moteFade = frame.starOpacity ?? (convergence || cosmic ? 0.75 : 0.12);
    const unchanged =
      previousFrameKey === frameKey && uniforms.uReveal.value === reveal;
    uniforms.uTime.value = frame.time;
    uniforms.uReveal.value = reveal;
    uniforms.uDissolve.value = dissolve;
    uniforms.uGrade.value = gradeValue;
    root.rotation.z = frame.time * (cosmic ? 0.025 : 0.004);
    root.position.z = cosmic ? local * 1.8 : 0;
    moteUniforms.uTime.value = frame.time;
    moteUniforms.uFade.value = moteFade * (1 - dissolve);
    bloom.strength =
      frame.bloomStrength ?? (cosmic ? 0.38 : convergence ? 0.2 : 0);
    grade.uniforms.uGrade.value = gradeValue;
    fog.far = frame.fogFar ?? (cosmic ? 90 : 240);
    const jumped =
        Number.isFinite(previousTime) &&
        Math.abs(frame.time - previousTime) > 0.5,
      stable =
        camera.position.distanceToSquared(previousPosition) < 1e-10 &&
        1 - Math.abs(camera.quaternion.dot(previousQuaternion)) < 1e-12;
    taa.accumulate = !jumped && stable && unchanged;
    temporal.enabled = cosmic;
    temporal.setFocusDistance(
      Math.max(1, camera.position.distanceTo(root.position)),
    );
    if (jumped) temporal.reset();
    composer.render();
    ...
    settledWaveKey =
      ...
      unchanged &&
      taa.accumulate &&
      (taa as TAARenderPass & { accumulateIndex: number }).accumulateIndex >=
        32 &&
      !temporal.enabled
        ? frameKey
        : null;
  ...
  const dispose = () => {
    if (disposed) return;
    disposed = true;
    if (loadedRoot) disposeObject(loadedRoot, gasMaterial);
    ...
    uniforms.uNebula.value?.dispose();
    uniforms.uHeight.value?.dispose();
    composer.dispose();
    renderer.dispose();
    loadedRoot = null;
  };
