...
export const FRONTEND_CUTOVER_BOUNDARIES: readonly FrontendCutoverBoundary[] = [
  ...
  { id: "games", complete: false, note: "Codenames presentation is source-owned, including header/clue flow, 5x5 board and reveals, turn footer, interactive key card, chat/clue composer, timed board overlays, help sheet and flying-card transition behavior. Cake Duel now source-owns its production cartridge controller, start/game/results route lifecycle, shipped locale-aware asset mapping, long-hover card preview/help content, palette and responsive geometry, card/hand rendering and pointer reordering, HUD, attack/block piles, deck, cake rail, claim/pick/pass/challenge action panel, claim/challenge/bout banners, transition-driven banner/Wolfy wiring, held pre-transition challenge board with revealed-pile orchestration, results pacing, Start/Results route decoration structure and action-error presentation. Remaining Cake Duel parity is exact spring/effect timing, including the precise shipped challenge pause/hold durations, plus any outcome-specific Results overlay asset that is still being verified against the shipped NormalApp export table; Chess and Pictionary also still need migration before Games can complete." },
