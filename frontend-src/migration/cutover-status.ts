...
export const FRONTEND_CUTOVER_BOUNDARIES: readonly FrontendCutoverBoundary[] = [
  ...
  { id: "messenger", complete: false, note: "Messenger owns IME-safe composition, duplicate-send fencing, failed-send draft preservation, scroll retention, image failure/preview focus restoration and short-window bubbles. Shipped bubble/thread interaction styling, avatar/photo focus behavior, mobile slide completion, typing/input/sealed-composer palettes and sealed choreography are source-bound and contract-checked; core palette/shadow values are now directly bound in the base component rather than relying on wrapper ancestry. Thread read_fact/unread_from/reread metadata, newest-message ordering, story-calendar timestamp handling, persistent signal.read facts, the shared local-read store and Signal Dock unread total follow the shipped runtime. Deterministic Chromium acceptance covers initial read/reread windows plus Daniel typing → sequential reply → evidence-file handoff and world-jump interruption. Source-owned shell notification queue and Signal artifact-delta arrival notifications have queue, deferral, focus and browser/unit coverage. The gate remains open for original-session visual comparison, original-agent/full-corpus/media sessions and original shell arrival timing; see docs/FRONTEND_COVERAGE_MATRIX.md." },
