...
const defaultCandidate = resolve(root, ".artifacts/build/app/cutover-candidate");
const defaultRollback = resolve(root, ".artifacts/build/app/cutover-rollback");
const candidateMarker = ".frontend-cutover-candidate.json";
const options = {
  build: resolve(root, ".artifacts/build/app"),
...
for (let index = 2; index < process.argv.length; index += 1) {
  ...
  if (argument === "--materialize") options.materialize = true;
    ...
    console.log(`Usage: node scripts/recovery/prepare_frontend_cutover_candidate.mjs [options]

  --build PATH       Vite application build (default .artifacts/build/app)
