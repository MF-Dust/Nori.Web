...
async function inspectGame(page, id, kind) {
  return page.locator(`.datasea-game[data-game="${id}"]`).evaluate((host, mode) => {
    ...
    while (stack.length) {
      ...
      if (fiber?.memoizedProps?.api && typeof fiber.type === "function") {
        ...
        if (mode === "discern") {
          const options = states.map((value) => value?.current).find((value) => Array.isArray(value) && value.some((entry) => typeof entry?.real === "boolean"));
          return options?.findIndex((entry) => entry.real) ?? -1;
        }
        if (mode === "resonance") {
          const refs = states.filter((value) => value && Object.hasOwn(value, "current"));
          const target = refs.find((value) => value.current?.tF)?.current;
          return { values: refs.slice(1, 4).map((value) => value.current), target };
        }
      }
...
export async function solveDataseaCoreGame(page, id) {
  ...
  if (await chrome.count()) {
    await game.locator("..").dispatchEvent("pointerdown", { pointerId: 1, clientX: 1, clientY: 1 });
    await page.clock.runFor(16);
  }
  const solved = async () => (await game.getAttribute("data-solved")) === "true";
  if (id === "denoise") {
    const canvas = game.locator("canvas"), box = await canvas.boundingBox();
    assert.ok(box && box.width > 100 && box.height > 100, "denoise has a usable canvas");
    ...
    for (let y = 2, row = 0; y < box.height; y += 22, row++) {
      ...
      await page.mouse.move(box.x + (row % 2 ? 2 : box.width - 2), box.y + y, { steps: 12 });
      await page.clock.runFor(16);
    }
    ...
    for (let frame = 0; frame < 2500 && !(await solved()); frame++) {
      const inBand = await game.evaluate((host) => {
        ...
        const cursor = [...host.querySelectorAll("div")].find((node) => node.style.width === "" && node.classList.contains("w-px"));
        if (!band || !cursor || band.style.opacity === "0") return false;
        return Math.abs(parseFloat(cursor.style.left) - parseFloat(band.style.left) - parseFloat(band.style.width) / 2) < 1.2;
      });
    ...
    for (let iteration = 0; iteration < 32 && !(await solved()); iteration++) {
      for (let index = 0; index < 3; index++) {
        ...
        const control = game.getByText(labels[index], { exact: true }).locator("..").locator("div").first();
        await control.click({ force: true });
        data = await inspectGame(page, id, "resonance");
        const delta = [data.target.r, data.target.phi, data.target.g][index] - data.values[index];
        const step = (index === 1 ? 0.05 : 0.03) * 0.3;
        const count = Math.round(Math.abs(delta) / step);
        await page.evaluate(({ count, positive }) => {
          for (let i = 0; i < count; i++) window.dispatchEvent(new KeyboardEvent("keydown", { key: positive ? "ArrowRight" : "ArrowLeft", shiftKey: true, bubbles: true }));
  ...
  assert.equal(await solved(), true, `${id} must solve through its original input handlers`);
