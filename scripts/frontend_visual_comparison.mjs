 ...
 *
 * Run: node scripts/frontend_visual_comparison.mjs
 */
...
const output = resolve(
  process.env.NORI_VISUAL_OUTPUT ?? "frontend-visual-comparison",
);
 ...
 * scripts/frontend_antivirus_probe.mjs does for the same component. The QTE is a
...
async function captureBoot(browser) {
  ...
  const ctx = await openHarness(browser, {
    ...
    source: "tests/frontend-boot-corruption-harness.tsx",
    stylesheet: true,
    cubism: true,
  });
  try {
...
async function captureEnding(browser) {
  ...
  const ctx = await openHarness(browser, {
    ...
    source: "tests/frontend-farewell-ending-harness.tsx",
    stylesheet: true,
    cubism: true,
    background: "#05080d",
  });
...
async function captureMemory(browser) {
  ...
  const ctx = await openHarness(browser, {
    ...
    query: "?scene=memory",
    source: "tests/frontend-memory-datasea-harness.tsx",
  });
  const { page } = ctx;
  try {
...
async function captureCultFlash(browser) {
  ...
  const ctx = await openHarness(browser, {
    ...
    source: "tests/frontend-scene-tools-harness.tsx",
    stylesheet: true,
  });
  try {
