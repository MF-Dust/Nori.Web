...
const output = resolve(".artifacts/games-lifecycle");
  ...
  },
  {
    ...
    async play(page, locale) {
      ...
      await page
        ...
        .selectOption("2");
      // Must not be a board word or a substring of one. "lantern" is rejected
      // whenever the dealt board contains ANT, LAN or a similar fragment, and
      // the click still succeeds, so the lifecycle then waits forever for a
      // dispatch that the validator never sent.
      await clue.fill("zzqxv");
      await page.getByRole("button", { name: "Send guess", exact: true }).click();
      const rejected = page.locator(".source-codenames-error");
      try {
        await rejected.waitFor({ timeout: 500 });
      } catch {
        return "clue submitted";
      }
      throw new Error(`clue rejected: ${(await rejected.textContent())?.trim()}`);
    },
  },
  {
    id: "pictionary",
