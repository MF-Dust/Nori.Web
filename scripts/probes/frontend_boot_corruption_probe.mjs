...
export async function verifyBootCorruption(
  ...
  try {
    ...
    const withPage = async (options, body) => {
      // One page at a time on the probe's browser. Eight live pages at once
      // exhausted WebGL contexts; closing the page before the next case does not.
      // A fresh browser per case was only paying the GPU process startup again.
