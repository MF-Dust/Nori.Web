...
const output = resolve(".artifacts/games-smoke");
const appHtml = await readFile(".artifacts/build/app/index.html", "utf8");
...
const sourceCss = (await Promise.all(appStyles.map(path => readFile(".artifacts/build/app" + path, "utf8")))).join("\n");
await mkdir(output, { recursive: true });
const compiled = await build({
  entryPoints: ["tests/frontend/harness/frontend-games-browser.tsx"], bundle: true, write: false,
