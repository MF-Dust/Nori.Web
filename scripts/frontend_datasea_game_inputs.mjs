...
async function rootFor(page, id) {
  ...
  assert.equal(
    await root.count(),
    1,
    `expected one mounted Datasea ${id} game`,
  );
  const titlebar = root
    .locator(
      "xpath=ancestor::*[contains(concat(' ', normalize-space(@class), ' '), ' datasea-wave-window ')][1]",
    )
    .locator(".datasea-wave-titlebar");
...
async function canvasBox(root) {
  ...
  assert.ok(
    ...
    "Datasea game canvas has no layout box",
  );
  return { canvas, box };
}
    ...
    for (let point = 0; point < 7; point += 1)
      for (const [a, b] of edges)
        if (
          a !== point &&
          b !== point &&
          distance(points[point], points[a], points[b]) < 0.055
        )
          return false;
    return true;
  };
  for (let attempt = 0; attempt < 240; attempt += 1) {
    ...
    for (let guard = 0; points.length < 7 && guard < 400; guard += 1) {
      ...
      if (
        points.every(
          (point) =>
            Math.hypot(candidate.x - point.x, candidate.y - point.y) >= 0.17,
        )
      )
    ...
    if (
      count >= 4 &&
      count <= 12 &&
      (!fallback || Math.abs(count - 8) < Math.abs(fallback.count - 8))
    )
      fallback = { points, count };
  }
...
async function solveUnknot(page, root) {
  ...
  const point = (normalized) => ({
    x: box.x + 30 + normalized.x * (box.width - 60),
    y: box.y + 30 + normalized.y * (box.height - 60),
  });
  for (let index = 0; index < initial.length; index += 1) {
    const from = point(initial[index]),
      to = point(target[index]);
...
const PATH_DIRECTIONS = new Map([
  ["M30 0V60", ["N", "S"]],
  ["M0 30H60", ["W", "E"]],
  ["M30 0A30 30 0 0 0 60 30", ["N", "E"]],
  ["M60 30A30 30 0 0 0 30 60", ["E", "S"]],
  ["M30 60A30 30 0 0 0 0 30", ["S", "W"]],
  ["M0 30A30 30 0 0 0 30 0", ["W", "N"]],
  ["M30 30H60", ["E"]],
  ["M0 30H30", ["W"]],
  ["M30 0V30", ["N"]],
  ["M30 30V60", ["S"]],
...
const rotateDirection = (direction, turns) =>
  DIRECTIONS[(DIRECTIONS.indexOf(direction) + turns) % 4];
...
async function solveRelay(page, root) {
  const board = await root.evaluate((element) => {
    ...
    for (const button of buttons) {
      ...
      tiles[index] = {
        ...
        turns:
          Number.parseInt(svg.style.transform.match(/-?\d+/)?.[0] ?? "0", 10) /
          90,
      };
    }
    ...
    return {
      tiles,
      inletRow: Math.round((Number.parseFloat(inlet.style.top) - 27) / 64),
      outletRow: Math.round((Number.parseFloat(outlet.style.top) - 24) / 64),
    };
  });
  const tiles = board.tiles.map((tile) => {
    const base = new Set();
    for (const path of tile.paths)
      for (const direction of PATH_DIRECTIONS.get(path) ?? [])
        base.add(direction);
    return { ...tile, base };
  });
  const compatible = (tile, incoming, outgoing) => {
    ...
    for (let turns = 0; turns < 4; turns += 1) {
      const rotated = new Set(
        [...tile.base].map((direction) => rotateDirection(direction, turns)),
      );
  ...
  const visit = (index, incoming, visited, path) => {
    ...
    for (const outgoing of outgoingOptions) {
      ...
      if (row === board.outletRow && column === 4 && outgoing === "E") {
        solution = [...path, { index, incoming, outgoing }];
        return;
      }
      ...
      visit(next, opposite(outgoing), nextVisited, [
        ...path,
  ...
  for (const step of solution) {
    ...
    for (let turns = 0; turns < 4; turns += 1) {
      ...
      if (rotated.has(step.incoming) && rotated.has(step.outgoing)) {
        desiredTurns = turns;
        break;
      }
    }
...
async function solveEcho(page, root) {
  ...
  for (let stage = 0; stage < 3; stage += 1) {
    ...
    for (
      let elapsed = 0;
      elapsed < 18000 && sequence.length < lengths[stage];
      elapsed += 25
    ) {
      const warm = await canvas.evaluate((element) => {
        ...
        return Array.from({ length: 6 }, (_, pad) => {
          ...
          const data = context.getImageData(
            Math.round((cx + Math.cos(angle) * radius - 3) * sx),
            Math.round((cy + Math.sin(angle) * radius - 3) * sy),
      ...
      if (quiet > 1600 && sequence.length < lengths[stage]) {
        sequence.length = 0;
        quiet = 0;
      }
      await advance(page, 25);
    ...
    assert.equal(
      sequence.length,
      lengths[stage],
      `echo stage ${stage + 1} rendered pulse sequence was not observed`,
    );
...
function normalizedPoint(box, x, y) {
  ...
  return {
    x: box.x + box.width / 2 + x * radius,
    y: box.y + box.height / 2 + y * radius,
  };
}
...
async function solveLure(page, root) {
  ...
  for (const waypoint of waypoints.slice(1)) {
    ...
    const steps = Math.max(
      2,
      Math.ceil(Math.hypot(next.x - previous.x, next.y - previous.y) / 6),
    );
    for (let step = 1; step <= steps; step += 1) {
      await page.mouse.move(
        previous.x + ((next.x - previous.x) * step) / steps,
        previous.y + ((next.y - previous.y) * step) / steps,
      );
...
function balanceTilt(assignment) {
  ...
  for (let index = 0; index < assignment.length; index += 1) {
    ...
    if (hook.bar === 1) {
      leftMoment += balanceWeights[index] * hook.x;
      leftWeight += balanceWeights[index];
    } else {
      rightMoment += balanceWeights[index] * hook.x;
      rightWeight += balanceWeights[index];
    }
  }
  const raw = [rightWeight - leftWeight, leftMoment, rightMoment],
    scales = [5, 2, 2],
    tolerances = [0.5, 0.12, 0.12];
  return raw.map((value, index) => {
    let degrees = 20 * Math.tanh(value / scales[index]);
    if (Math.abs(value) > tolerances[index])
      degrees = Math.sign(value) * Math.max(Math.abs(degrees), 5);
    return (degrees * Math.PI) / 180;
  });
}

function balanceGeometry(box, assignment) {
  const scale = Math.min((box.width / 2 - 38) / 1.78, box.height / 3.4),
    centerX = box.width / 2;
  const top = box.height * 0.09,
    level = 0.55 * scale,
    drop = 0.95 * scale,
    weightDrop = 0.42 * scale;
  const angle = balanceTilt(assignment),
    pivot = { x: centerX, y: top + level };
  const left = {
    x: pivot.x - scale * Math.cos(angle[0]),
    y: pivot.y - scale * Math.sin(angle[0]),
  };
  const right = {
    x: pivot.x + scale * Math.cos(angle[0]),
    y: pivot.y + scale * Math.sin(angle[0]),
  ...
  const bars = [
    { x: left.x, y: left.y + drop },
    { x: right.x, y: right.y + drop },
  ];
  const hooks = balanceHooks.map((hook) => ({
    x: bars[hook.bar - 1].x + hook.x * scale * Math.cos(angle[hook.bar]),
    y: bars[hook.bar - 1].y + hook.x * scale * Math.sin(angle[hook.bar]),
  }));
  const weights = assignment.map((hook, index) => ({
    x: hooks[hook].x,
    y:
      hooks[hook].y +
      weightDrop +
      0.092 * scale * Math.sqrt(balanceWeights[index]),
  }));
...
async function solveBalance(page, root) {
  ...
  for (const [weight, target] of [
    ...
    const geometry = balanceGeometry(box, assignment),
      from = geometry.weights[weight],
      to = geometry.hooks[target];
