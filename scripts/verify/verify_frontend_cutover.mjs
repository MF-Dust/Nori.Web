...
import { historicalAssetReferences } from "../lib/frontend_asset_ownership.mjs";
...
if (idleBoundaryMatch[1] === "true") {
  const idleScreen = await readFile("frontend-src/screens/idle-screen.tsx", "utf8");
  const loader = await readFile("frontend-src/apps/marginal-growth/loader.ts", "utf8");
  if (!idleScreen.includes("MarginalGrowthRibbonView")) {
    throw new Error("idle-qfr is marked complete but IdleScreen does not mount MarginalGrowthRibbonView");
  }
  if (!loader.includes("marginal-growth-cache-")) {
    throw new Error("idle-qfr is marked complete but the ribbon loader does not fetch the shipped cache");
  }
}
