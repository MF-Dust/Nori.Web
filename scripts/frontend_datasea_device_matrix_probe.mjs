 ...
 *
 * Additive companion to scripts/frontend_memory_datasea_probe.mjs. Nothing in
 ...
 * built .frontend-app-build/assets/index-*.js): quality | performance |
...
const DATASEA_HARNESS = "tests/frontend-memory-datasea-harness.tsx";
const STAGE_HARNESS = "tests/frontend-boot-corruption-harness.tsx";
const TUNER_HARNESS = "tests/frontend-debug-harness.tsx";
 ...
 * timeline is jumped exactly like scripts/frontend_memory_datasea_probe.mjs:
...
export async function verifyDataseaDeviceMatrix(
  ...
  await onPage(
    ...
    async ({ page, errors, shaderErrors }) => {
      ...
      // (tests/frontend-memory-datasea.test.ts asserts dataseaCamera(13).cameraRot.x
