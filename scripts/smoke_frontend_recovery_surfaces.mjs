...
const groups = {
  ...
  "memory-datasea": ["./frontend_memory_datasea_probe.mjs", "verifyMemoryDatasea"],
  "farewell-ending": ["./frontend_farewell_ending_probe.mjs", "verifyFarewellEnding"],
};
const selected = process.argv[2];
if (!Object.hasOwn(groups, selected)) throw new Error(`Unknown recovery surface: ${selected}`);
const output = resolve("frontend-surfaces-smoke", selected);
await mkdir(output, { recursive: true });
const server = await createServer({ configFile: "frontend-src/app.vite.config.ts", server: { host: "127.0.0.1", port: 47174, strictPort: true, hmr: false } });
let browser;
try {
  await server.listen();
