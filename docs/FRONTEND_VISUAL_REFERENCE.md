...
The isolated-world capture passed in job `105671448281` (run `35366900007`, head `18cba54`, merge `58d118beece37aa72c0e767e43817c539eabfd77`, artifact `10557051349`). Both targets report the unread Credits tooltip at opacity 1 with empty page errors and failed assets. Visual inspection nevertheless found the candidate badge transparent: its recovered DOM lacked shipped `iY` paint and `rY` anchoring. Those shared numeric/dot badge styles are now restored, and the probe also verifies the computed red gradient; DOM presence alone is insufficient. Final four-game review confirmed the preceding fixes took effect. Remaining differences include Pictionary cover vertical spacing/texture, simplified Chess SVG details, decorative compass/frame details and animation/random-frame variance. These remain documented visual limits.
