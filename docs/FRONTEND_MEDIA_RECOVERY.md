...
- The temporary boxed conversation UI has been replaced by the shipped floating bubble presentation. Recent messages expire after 30 seconds, at most three ordinary bubbles remain visible, and reconnect history stays silent. The composer uses the original mint/gray palette, Dock-centered placement, focus treatment and Ctrl/Cmd+K/Escape behavior. Speech activation is now in Sound settings. See `FRONTEND_CONVERSATION_RECOVERY.md` for the remaining chip/story/audio choreography.
