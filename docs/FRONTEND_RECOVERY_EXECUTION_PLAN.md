...
在 `dev/frontend-restoration-completion` 上继续实施了共享通知、Signal artifact 到达观察、Messenger pending-focus、窗口/Browser 生命周期、订阅清理、窄 Settings、损坏 PDF、音频焦点解锁，以及 Live2D idle/sleep crossfade 与 lip-form Debug 实时调参等修复。通知与 Messenger surface 已有单元/Chromium 证据；Farewell/Ending 资源重试和隐藏页 readiness 也有 focused probe 证据。当前源码还明确提供 Datasea 三波/十二游戏 probe、source-owned Debug 面板与 scene editor，以及 Codenames/Pictionary/Chess/Cake Duel 的游戏 runtime/presentation。五项 cutover gate 仍按实际证据保持 `false`，本批次没有提前切换 `public/index.html`。`frontend:recover:check` 仍需在最终合并前重复，任何 shipped marker 失败都按证据修正而不是放宽检查。
...
- 当前源码状态以 HEAD `085bad3` 为准；`7de88a5` 与 `94107bbf0b33911c519b50670664115c1a4efd47` 仅作为历史审计节点保留。
...
- `story-director.ts` 定义七段剧情优先级，`NoriFrontendRuntime` 将七个 ID 全部传给 director，`story-scenes.tsx` 已挂载 Boot、Corruption、Cult、Memory、Datasea、Farewell、Ending 七个 producer。注册和完成事实已有源码/单元证据；六段非 Cult 仍分别结算原版视觉、浏览器、语料、媒体与私有代理验收。
- 场景编辑器、冷开场图形组件和腐化小游戏预览已有 source-owned 实现；这些实现不能由此推定六段非 Cult 已达到原版 parity。
- Messenger 最新已恢复 GFM 删除线、任务列表、自动链接、消息气泡配色与阴影、会话行状态和键盘焦点。这些作为保留回归项。
- 文档包含分轮历史记录，部分“仍未恢复”表述已过期。状态冲突以当前源码、当前测试和运行证据为准，先核对再修正文档。

### 2.2 历史 CI 记录（非 HEAD `085bad3` 新结果）

初始失败审计对象为 [Cloudflare Worker run 35285404971](https://github.com/MF-Dust/Nori.Web/actions/runs/35285404971)；该 run 的 cold-open 错误路径已由 P0 修复并在 run `35354435518` 通过。下表保留历史运行/Job 记录，不能解读为 HEAD `085bad3` 新跑出的通过结果；表中的失败与通过均是历史节点，当前源码覆盖状态见本节末及 P3。
...
调用链涉及 `verifyColdOpen`、`verifyNoriScene` 和 `smoke_frontend_app.mjs`。根因是同一浏览器会话中的已解码图片/资源状态越过失败注入边界；修复将错误与重试路径隔离到新浏览器 context，同时保留真实请求命中、error、retry、释放和迟到回调断言。后续 Debug 观察器曾因旧 harness 不提供诊断 facade 而把成功模型载入转成 error；`NoriStage` 现把该观察器作为可选附属接口，四个目标 surface 的历史 head 复验记录保留在 CI 记录中。

所有后续回归继续保留失败资源路径和真实 renderer 断言，不以跳过或放宽断言换取通过。当前源码事实另行记录：Datasea probe 覆盖三波与十二游戏，但这不是新的 CI 通过声明。
...
| P3 | 六段非 Cult 剧情的原版 parity 逐段还原 | P2 | 各段完成独立 parity 验收 |
...
默认按上述阶段推进。独立任务可在外部阻塞期间前移，记录原因；不得以小型样式提交持续替代六段非 Cult 剧情的 parity 主线。实施不引入按天工期承诺，进度按工作包验收结算。
...
- [x] P1-07 明确六段非 Cult 剧情与游戏的代理请求、返回、媒体和完成事件；列出所需真实会话或可追溯记录。
...
- [x] P2-08 将六段非 Cult 剧情需要的状态通道接入已有 StoryClock、StoryAudio、scene store 和 director；复用其暂停、实例隔离与确认机制。
...
部分状态：Messenger 独立 Chromium 在 `c9fb95c4` 对应 run `35358775277` 通过（历史记录），运行时用例覆盖 IME、草稿保持、滚动和历史静默。七个 producer 已接入共享 clock/audio/scene/director，六段非 Cult 仍缺原版 parity；未完成项集中在完整语料/附件矩阵、浮动区原版视觉、私有代理媒体编排及 P2-10 至 P2-12 的全部组合验收。

## 7. P3：六段非 Cult 剧情的原版 parity

七个 producer（Boot、Corruption、Cult、Memory、Datasea、Farewell、Ending）已在 HEAD `085bad3` 的源码调用链注册；本阶段剩余的是六段非 Cult 的参考拆解、原版时间线/视觉/媒体对照、独立验收和整站回归。现有 cult-flash 作为持续回归项。共享渲染部分优先复用，场景拥有自己的生命周期。下列未勾选项是 parity/验收缺口，不是缺少 producer 注册。
...
- [ ] P3-B04 完成真实模型关键帧、首次进入/重进、错误重试、完成确认和世界替换的原版 parity 验收。
...
- [ ] P3-C06 验证所有分支、窄窗口、键盘/指针、提前退出、同世界重载与完成确认的原版 parity。
...
- [ ] P3-M04 验证窗口堆叠、焦点、各输入门、恢复与取消、后台暂停和完成事实的原版 parity。
...
- [ ] P3-X03 七段联合验证优先级和交接；未满足 parity/验收条件的前置剧情不会被跳过或伪造完成。
- [x] P3-X04 记录正式注册证据和源码路径，剧情完成状态与真实代理验收状态独立呈现。

部分状态：七个 producer 均已注册；六段非 Cult 均有源码时间线/渲染器，12 项剧情测试通过。历史 head `f7a7fa6683adc8c9088d416a2fa6784c94464ac1` 的 Boot/Corruption 与 Farewell/Ending 浏览器记录保留在 2.2；它们不是 HEAD `085bad3` 的新 CI 结果。当前 Datasea source/probe 覆盖三波与十二游戏，静态文字已按 shipped 证据接入源码；原版消息窗口/compositor、逐帧视觉、媒体和私有代理原文/语音仍是独立缺口，因此 B/C/M/D/F/E 与 X01–X03 的复合任务不勾选。

## 8. P4：游戏完整表现与联调

| 游戏/范围 | 待执行内容 | 完成证据 |
| --- | --- | --- |
| Codenames | 已有 deterministic 13-step tutorial/gates、results 和 sudden-death presentation；仍需原版脚本代理叙事/语音、剩余 start/results 装饰动画与完整生命周期 | 教程至自由模式、胜败分支、原版画面对照、事件/语音顺序 |
| Chess | 已有完整 22-ply guided opening；仍需结果/覆盖层时序、代理语音编排、视觉对照和完整生命周期 | 保留 22 半回合教程，普通对局/结果/重连/历史浏览和原版语音案例 |
| Pictionary | 已有 cover/help/results surfaces、绘图与提示 runtime；仍需原版装饰/动画、模型表情、真实代理快照推理和完整生命周期 | 双角色、多轮、skip/未完成、绘图快照与真实代理响应 |
| Cake Duel | 已有 source runtime/controller 与 start/game/results binding；仍需整站生命周期、视觉和原版代理/媒体验收 | 开始/对局/结果、challenge/Wolfy/翻牌、语种资源与退出清理 |
...
- [ ] P4-02 验证 Codenames deterministic tutorial 链路并接入兼容原版脚本 backend/agent，区分本地 `free_play` 现状与原版脚本状态。
...
- [ ] P4-06 完成 Cake Duel 完整桌面生命周期、视觉和原版代理/媒体验收，避免为了已完成 source runtime 再次大规模重写。
...
部分状态：22 项游戏测试、cartridge 测试及主游戏浏览器检查属于当前/历史混合证据；当前源码明确包含 deterministic Codenames tutorial/results、Pictionary help/results、Chess 22-ply tutorial 和 Cake Duel source runtime。四款游戏的完整生命周期、原版视觉对照、双语/窗口矩阵与原版代理推理/语音仍未关闭，因此 P4 复合任务保持未完成。

## 9. P5：Debug、高级编辑器与辅助界面

Debug 面板和 scene editor 在 HEAD `085bad3` 已由 source-owned 组件承载；P5-01 至 P5-03 的未勾选状态表示原版布局/扩展对照尚未完成，不表示这些面板尚未接入源码。

- [ ] P5-01 逐项对照 Network、Compute、Gesture、Reaction 实验面板的控制项、状态与回调，并记录原版布局差异。
- [ ] P5-02 验证 source-owned 游戏情景注入与原版支持的调试工具，使用隔离世界确认释放和失败边界。
- [ ] P5-03 为已注册的七段 producer 核对高级通道、调参控件、范围校验与预览；剩余项是原版视觉/时序 parity。
...
历史记录与当前源码状态：Network fault、真实 Idle ledger、Gesture、Reaction、Codenames scenario、Shatter 与 Datasea tuner 已接实际运行时；Debug Chromium job `105644556047` 通过压缩 GLB、范围约束、持久化、失败重试、剧情接管与资源释放。对 `Debug-D6AtxpLT.js` 的注册项审计确认原版没有其他「按剧情分组」的专属 tuner。全页 Glitch 已接生产 filter；33 个 Chess 与 4 个 Cake Duel 原版 scenario ID 已进入 UI，并由 controller/backend 的真实 `debugLoadScenario` 命令执行。Live2D 使用当前挂载的生产模型执行插件、rest pose、expression 和 motion；Audio 直接修改持久化设置并同步 mixer/speech；Pat 参数直接作用于生产 recognizer、spring、输入区域和 friction synth；Reactions 覆盖生产 director 已绑定的 28 个 Pictionary/Chess/Codenames 事件；Notifications 执行真实 `notification.debug.push` 往返并观察 `notification.pushed`。Inject Talk 与 Nori Context 所需的私有 handler 不存在，因此只呈现可观测会话状态和明确阻塞，不伪造代理输出或上下文。Live2D idle/sleep crossfade、mouth-open/mouth-form、表达式 lip-sync share 已接入当前挂载的生产模型，并保留原版范围与 48 ms talking trace。Pat armed/state-gate/head-zone/model-pointer telemetry 已接回生产输入和模型投影。Audio manager 的 context Resume/Suspend、loaded music/SFX、BGM play/pause/resume/stop/seek/crossfade、active SFX、HRTF listener/source/distance telemetry，以及 Speech/Music/SFX 三轨 room/hall/cave reverb、wetness、low/high/band-pass filter 已接回 session-owned AudioMixer，并复刻 shipped equal-power effect graph。Cake Duel reaction catalog、挑战前后反应、claim tell、last-cake mood、Wolfy/cake/game-end 事件，Codenames sudden-death persistent mood，以及 Debug 强制 variant、冷却绕过、persistent mood、tell sampling 已接回同一个生产 reaction director；exclusive 游戏窗口不再误封锁游戏 reaction。P5 现主要剩原版布局与跨场景视觉/时序对照。通知 tab 已能观察 source shell queue，queue/dismiss 的完整原版调试布局仍需对照。上述历史路径已有 106 项聚合运行时测试；Debug Chromium job `105657772066` 通过 UI 到 facade 的交互合同（历史记录，不代表 HEAD `085bad3` 新结果）。Worker job `105661992194` 的 `Nori scene and cold-open lifecycle` 步骤使用真实 NoriStage 模型，通过 catalog、physics 切换/恢复和 HeadPat 参数恢复断言；该 job 随后的 Scene editor 步骤发现新 Audio Debug tab 漏掉真实 `Corrupt voice` checkbox。控件现已恢复并接回 DebugScreen 的 scene lease，保留剧情接管、世界切换及关闭 Debug 清理；现有浏览器断言未放宽，并由 Worker job `105667661060` 的 Scene tools 与完整 Scene editor 步骤复验通过。由于原版布局与跨场景视觉/时序对照仍缺，P5-01 至 P5-03 暂不关闭；source ownership 已完成，剩余是 parity 验收。P5-06 至 P5-08 的辅助应用完整矩阵也未因单次候选截图而关闭。

## 10. P6：原版代理与媒体验收

- [x] P6-01 从 P1 的依赖列表确认可用的原版代理/媒体环境、版本及测试会话；明确仅具本地模拟条件的接口。
- [ ] P6-02 验证 pat 请求 → 回复 → 媒体 → 表情/嘴型 → 播放完成的顺序。
- [ ] P6-03 验证腐化交互、六段非 Cult 剧情的揭示/打断与语音门，以及故障/重连分支。
...
部分状态（历史 CI 记录）：历史资产扫描器 3 项与 Worker payload guard 通过。719b9993、c7714a42 的候选完整 job 通过，包含静态浏览器、五组系统截图和 Worker dry-run。ca657 的截图已人工审阅并修复可见差异，记录见 FRONTEND_VISUAL_REFERENCE.md；About 参考截图发现透明窗口采样问题，四游戏截图存在历史 Pictionary 标题选择器歧义，均记为历史复采事项。完整功能/姿态/场景矩阵未齐备，P7 复合项保持未完成。
...
| live2d | P3 六段非 Cult 的原版 parity、专用渲染对照、手势粒子对照，P6 相关反馈，P7 资源与视觉 |
...
部分状态（历史 CI 记录）：隔离候选可生成 symlink/materialized 两种形式，materialized 产物无 symlink，rollback manifest/hash 与静态入口 smoke 已通过；候选 Worker 配置从生产 JSONC 临时生成且不改生产配置。719b9993、c7714a42 的候选静态入口、系统配对截图、Worker dry-run 与静态资产 payload guard 均已通过；后续视觉步骤失败也不再跳过 dry-run。原版/源码视觉复验、完整资产/R2 运行检查和真实回退浏览器证据仍阻塞 P8-02、P8-06、P8-07。
...
- [ ] 七个 story producer 均已注册；六段非 Cult 已完成源码重建但仍待原版 parity 验收，已有 cult-flash 无回归。
...
2026-09-18（历史记录）：在原 PR 分支继续实施。完成覆盖矩阵、六段非 Cult 正式剧情源码接入、Messenger 交互修复、游戏表现与 Debug 实际运行时控制、Credits SVG 与模型通道、历史 chunk 扫描扩展。各项完整验收包含的视觉/浏览器要求尚未全部满足，因此不因代码存在批量勾选。

新增检查：`frontend:stories:test`、`frontend:stories:smoke`、`frontend:ownership:test`，以及按 cold-open、Messenger、Boot/Corruption、Memory/Datasea、Farewell/Ending、Debug 隔离的 Actions 浏览器任务。当前本地运行时 126 项、剧情 12 项、游戏 22 项、扫描器 3 项通过。浏览器结果和未通过步骤按下述工作包记录，不把“有检查”写成“已验收”。

2026-09-25（HEAD `085bad3` 状态同步）：七个 story producer 已注册；六段非 Cult 仍待原版 parity。Datasea source/probe 覆盖三波与十二游戏，Debug 面板/scene editor 和四款游戏的指定 source runtime 已明确；剩余 full lifecycle/visual/agent/media 与 private handler/layout 验收仍开放。本文不新增 CI 通过声明，五个 false 门禁保持不变。

历史 CI 记录（P0）：修复提交 `55c79a86715c807ea8a7dfd906e09f598b78ae21` 的 [CI 35354435518](https://github.com/MF-Dust/Nori.Web/actions/runs/35354435518) 已成功，含整站 smoke。独立 browser context 消除了已解码图片缓存对失败注入的干扰，保留成功、失败、重试、释放及迟到回调断言。整合提交 `3e2808de355b1cd18a762f2e22683eb72c5f0df8` 的 cold-open 和 Boot/Corruption 浏览器任务通过，其余新矩阵失败仍在修复。

历史运行记录：`c9fb95c4b8522c9902e9fe5b507759f0fb6b0c5c` 的独立矩阵中 cold-open、Boot/Corruption、Messenger、Debug 已通过；Debug job `105644556047` 实际加载 Meshopt 压缩 Datasea GLB，并覆盖调参、错误重试、接管与释放。此后 `f7a7fa6683adc8c9088d416a2fa6784c94464ac1` 的 surface run `35364523143` 再次通过 Debug、cold-open、Boot/Corruption、Farewell/Ending 与 Messenger；Datasea games job `105663601161` 已通过全部 12 项真实输入；完整 Memory/Datasea 交接由后续提交复验。此前 Worker job `105661992194` 的真实 NoriStage 模型步骤通过，随后 Scene editor 因新 Audio tab 遗漏 `Corrupt voice` checkbox 失败。`7de88a5` 已恢复该真实 scene 控件并保持原浏览器功能断言；Worker run `35365741254` 的 job `105667661060` 随后通过真实 NoriStage/cold-open、Scene editor/Scene tools、Preview、Messenger、Chip 与最终整站 smoke。
### 当前工作包与证据归属

| 工作包 | 源码/脚本交付 | 已取得证据 | 尚未关闭的验收 |
| --- | --- | --- | --- |
| P0 | 独立浏览器上下文错误注入 | 历史：55c79a8 整站 CI 通过；3c73c80 独立检查通过 | 已关闭 |
| P1 | 15 应用覆盖矩阵 | 注册表与实际 source session 接线核对 | 已有配对截图；逐功能/场景参考索引仍不完整 |
| P2 | Messenger 交互、媒体失败、场景通道 | 历史：Messenger 独立 Chromium 通过；运行时测试 | 原版代理媒体、完整语料与浮动区视觉对照 |
| P3 | 七个已注册 producer；六段非 Cult source timelines/renderers | 12 项剧情测试；历史 f7a7fa6 的 Boot/Corruption、Farewell/Ending 浏览器记录 | Datasea 三波/十二游戏 source/probe 已覆盖；静态文字已按 shipped 证据接入；原版消息窗口/compositor、视觉、媒体与私有代理语音 |
| P4 | Codenames deterministic tutorial/results、Pictionary help/results、Chess tutorial、Cake Duel source runtime 与模型反应 | Python cartridge、22 项游戏测试及主游戏浏览器历史记录 | 四游戏完整生命周期、双语/全部状态视觉对照；原版代理推理/语音/媒体 |
| P5 | Source-owned Debug 面板与 scene editor；网络/算力/情景控制、Shatter/Datasea 调参、生产 Glitch/Live2D/Audio/Pat/Reaction/通知诊断 | 126 项运行时测试、cartridge 隔离实例测试及历史 Debug jobs | 原版布局对照与 private Inject Talk/Nori Context handlers；不再把已接入的 manager/model telemetry 记为源码缺口 |
| P6 | 保留接口、请求和媒体生命周期 | noop 位置与真实所需输入已记录 | 原版代理实现/授权测试会话 |
| P7 | 独立矩阵、原版/源码配对截图脚本 | 历史资产引用检查、固定环境脚本和成功截图 | About及四游戏已取得可审阅配对并修差异；世界隔离/样式复采及完整矩阵待最终检查 |
| P8 | 隔离候选入口、静态资源/回滚哈希校验 | 历史：本地候选、14 个回滚文件、静态入口 smoke 与 Worker dry-run 通过 | 最终修订视觉与完整资源/回退浏览器；全部功能门禁前置条件 |
