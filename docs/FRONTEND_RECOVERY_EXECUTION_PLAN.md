...
- [ ] P0-01 获取 PR 最新 head、base、工作区状态和仓库约束；建立对应分支工作区，保留所有已有修改。
...
- [ ] P1-01 从 app 注册表、路由、窗口类型、shipped chunk 导入关系和动态入口枚举前端功能，覆盖主路径与隐藏入口。
...
- [ ] P1-07 明确六段剧情与游戏的代理请求、返回、媒体和完成事件；列出所需真实会话或可追溯记录。
...
- [ ] P8-04 扩展现有 cutover 校验对所有历史 chunk/间接导入的覆盖，保留明确的供应商运行时边界。
