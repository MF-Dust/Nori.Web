...
- Head 为 `94107bbf0b33911c519b50670664115c1a4efd47`，包含最新 Messenger 会话行交互状态恢复。
...
- `story-director.ts` 已定义七段剧情优先级；`story-scenes.tsx` 当前仅挂载 `cult-flash`。六段剩余剧情必须分别完成正式挂载、完成事实和生命周期验收。
...
- [ ] P1-08 将审计中新发现的内容加入台账，指定 P2–P8 归属和验收条件，禁止仅写“后续优化”。
...
- [ ] P2-03 保留最新 GFM、链接跳转、会话行状态、消息气泡配色/阴影与 focus-visible 契约，并用实际 DOM 和交互验证关键结果。
...
- [ ] P2-08 将六段缺失剧情需要的状态通道接入已有 StoryClock、StoryAudio、scene store 和 director；复用其暂停、实例隔离与确认机制。
...
- [ ] P3-X04 记录正式注册证据和源码路径，剧情完成状态与真实代理验收状态独立呈现。
...
- [ ] P5-04 回归 JSON 导入导出、100 KB 限制、重复阶段 ID、零时长门、排序/删除、音轨偏移和暂停 seek。
- [ ] P5-05 验证预览/调试对正式剧情的让位、关闭与切换世界清理，以及不会误提交剧情完成事实。
...
- [ ] P6-01 从 P1 的依赖列表确认可用的原版代理/媒体环境、版本及测试会话；明确仅具本地模拟条件的接口。
...
- [ ] P6-07 对无法运行的案例保留“外部阻塞”，写明最小所需资料和解除条件，继续完成其余独立任务。
...
| P2 | Messenger 交互、媒体失败、场景通道 | Messenger 独立 Chromium 通过；运行时测试 | 原版代理媒体与全内容语料 |
| P3 | 六段正式剧情及专用渲染 | Boot/Corruption 浏览器通过并人工检查模型画面 | 其他场景浏览器复验；十二小游戏实际输入；原版视觉 |
| P4 | 教程 reducer、四游戏表现、模型反应 | Python cartridge 与 20 项游戏测试通过 | 扩展浏览器及原版代理推理/语音 |
| P5 | 实际网络/算力/情景控制、Shatter/Datasea 调参 | 92 项运行时测试包含真实账本与生命周期 | 调参浏览器复验与原版界面对照 |
| P6 | 保留接口、请求和媒体生命周期 | noop 位置与真实所需输入已记录 | 原版代理实现/授权测试会话 |
| P7 | 独立矩阵、原版/源码配对截图脚本 | 资产引用检查和已有成功截图 | 配对截图检查及余下 CI |
| P8 | 隔离候选入口、静态资源/回滚哈希校验 | 本地候选与14个回滚文件校验通过 | 候选浏览器、Worker dry-run、全部功能门禁前置条件 |
