"""Small local Better-Auth-compatible session surface.

It implements only the endpoints the public NoriOS web client calls.  OTPs
are local development credentials (shown as `123456` by default); no email is
sent and no upstream account is accessed.
"""
...
def _cookie_token(request: Request) -> Optional[str]:
    # Better Auth's cross-domain plugin sends a synthetic Better-Auth-Cookie
    # request header; native local requests use Cookie.
...
async def convex_token(request: Request):
    ...
    if session is None:
        return {"token": None}
    # Local-only token. The local frontend bridge does not grant this token
    # access to any remote Convex deployment.
