...
test("ending camera interiors follow the shipped smoothstep, not a power ease", () => {
  const shipped = readFileSync("public/assets/NormalApp-Cn6agT0F.js", "utf8");
  assert.match(shipped, /DZ = \(t\) => t \* t \* \(3 - 2 \* t\)/);
  const smooth = (value: number) => value * value * (3 - 2 * value);
  const power2InOut = (value: number) =>
    value < 0.5
      ? Math.pow(value * 2, 3) / 2
      : 1 - Math.pow((1 - value) * 2, 3) / 2;
  const start = 21.7;
  const end = 26.9;
  const time = start + 0.25 * (end - start);
  const progress = smooth(0.25);
  assert.ok(Math.abs(progress - power2InOut(0.25)) > 0.05);
  const camera = endingCamera(time).camera;
  const expectedY = 1.1 + (0.8 - 1.1) * progress;
  assert.ok(
    Math.abs(camera.y - expectedY) < 1e-9,
    `camera y ${camera.y} left the shipped smoothstep ${expectedY}`,
  );
});
